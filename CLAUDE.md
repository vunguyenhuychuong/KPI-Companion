...
- [x] **Autonomous Agent Loop** — service nền `services/autonomous_agent.py` chạy Perceive → Reason → Act → Remember theo chu kỳ; ghi `AgentCycleLog` và bản ghi nội bộ phục vụ Autonomous Inbox/Journal, nhưng không hiển thị như một phiên trong Chat AI; không tự confirm hay ghi KPI/Objectives/Work items.
...
- [x] **Daily Connected Source Scan** — Agent tự chủ quét các nguồn OAuth đã kết nối mỗi ngày, dùng AI tách bằng chứng công việc, lọc mục không liên quan KPI/độ tin cậy thấp/trùng lặp, tạo nháp trong Nhật ký công việc và gửi email cho user Gmail khi có nháp mới; chỉ cộng tiến độ KPI sau khi user xác nhận trong Journal.
