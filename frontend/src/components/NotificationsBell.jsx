...
const TYPE_ICON = { behind: '📉', deadline: '⏳', overdue: '⌛', runrate: '🔮' }
        ...
        <div className="notif-panel">
          ...
          {visible.length === 0 ? (
            ...
            <div className={`notif-item ${read.includes(n.id) ? '' : 'unread'}`} key={n.id}>
              <span className="notif-ico" style={{ color: SEV_COLOR[n.severity] }}>{TYPE_ICON[n.type] || '•'}</span>
              <div className="notif-body">
                <div className="notif-name" title={n.title}>{n.title}</div>
                <div className="notif-msg">{tr('notif.msg_' + n.type, n.params)}</div>
              </div>
              <button className="notif-x" onClick={() => dismiss(n.id)} title={tr('notif.dismiss')}>✕</button>
            </div>
          ))}
        </div>
      )}
