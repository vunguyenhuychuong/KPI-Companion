...
def _check_group_weight(
        db: Session, new_weight: float, objective_id: int | None, exclude_id: int | None = None
):
    """Tong trong so KPI TRONG CUNG MOT muc tieu (hoac nhom chua gan) khong vuot 100%."""
    q = select(models.KPI).where(models.KPI.archived == False)  # noqa: E712
    q = q.where(
...
def create_kpi(payload: schemas.KPICreate, db: Session = Depends(get_db)):
    _check_group_weight(db, payload.weight, payload.objective_id)
...
async def import_kpis(file: UploadFile, db: Session = Depends(get_db)):
    ...
    _check_group_weight(db, sum(p["weight"] for p in parsed), objective_id=None)
    created = []
    for p in parsed:
...
def confirm_kpi_proposal(payload: schemas.KPIProposalConfirm, db: Session = Depends(get_db)):
    ...
    try:
        ...
        for p in payload.kpis:
            if p.objective_id is not None and not db.get(models.Objective, p.objective_id):
                raise HTTPException(404, f"Không tìm thấy mục tiêu #{p.objective_id}")
            kpi = models.KPI(
                user_id=1, name=p.name, description=p.description, target=p.target,
        ...
        for k in db.scalars(select(models.KPI).where(models.KPI.archived == False)):  # noqa: E712
...
def balance_weights(payload: schemas.BalanceRequest, db: Session = Depends(get_db)):
...
def update_kpi(kpi_id: int, payload: schemas.KPIUpdate, db: Session = Depends(get_db)):
    ...
    _check_group_weight(db, final_weight, final_obj_id, exclude_id=kpi_id)

    # doi muc tieu (objective): ghi log bang TEN cho de doc; clear_objective=true -> go khoi muc tieu
    if moving:
...
def all_changelog(limit: int = 300, db: Session = Depends(get_db)):
...
def archived_kpis(db: Session = Depends(get_db)):
    """Danh sach KPI da go bo (de xem lai / khoi phuc)."""
    return list(db.scalars(select(models.KPI).where(models.KPI.archived == True)))  # noqa: E712


@router.post("/{kpi_id}/restore", response_model=schemas.KPIOut)
def restore_kpi(kpi_id: int, db: Session = Depends(get_db)):
    """Khoi phuc KPI da go bo. Neu trong so cu lam nhom vuot 100% -> tu ha xuong phan con trong."""
    kpi = db.get(models.KPI, kpi_id)
    if not kpi or not kpi.archived:
